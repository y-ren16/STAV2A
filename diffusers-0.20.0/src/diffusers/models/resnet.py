...
class FDN(nn.Module):
    def __init__(self, norm_nc, label_nc, num_groups, eps, affine=True):
        super().__init__()
        ks = 3
        pw = ks // 2
        self.param_free_norm = torch.nn.GroupNorm(num_groups, norm_nc, eps, affine=False)
        self.conv_gamma = torch.nn.Conv2d(label_nc, norm_nc, kernel_size=ks, padding=pw)
        self.conv_beta = torch.nn.Conv2d(label_nc, norm_nc, kernel_size=ks, padding=pw)
...
class ResnetBlock2DFDN(nn.Module):
    r"""
    A Resnet block.
    ...
    def __init__(
        ...
        *,
        ...
        self.in_channels = in_channels
        out_channels = in_channels if out_channels is None else out_channels
        inject_channels = in_channels if inject_channels is None else inject_channels
        self.out_channels = out_channels
        ...
        # else:
        #     self.norm1 = torch.nn.GroupNorm(num_groups=groups, num_channels=in_channels, eps=eps, affine=True)
        self.norm1 = FDN(in_channels, inject_channels, num_groups=groups, eps=eps, affine=True)
        ...
        # else:
        #     self.norm2 = torch.nn.GroupNorm(num_groups=groups_out, num_channels=out_channels, eps=eps, affine=True)
        self.norm2 = FDN(self.out_channels, inject_channels, num_groups=groups, eps=eps, affine=True)

        self.dropout = torch.nn.Dropout(dropout)
        conv_2d_out_channels = conv_2d_out_channels or out_channels
        self.conv2 = zero_module(
            LoRACompatibleConv(out_channels, conv_2d_out_channels, kernel_size=3, stride=1, padding=1)
        )
    ...
    def forward(self, input_tensor, local_conditions, temb):
        # import pdb; pdb.set_trace()
        hidden_states = input_tensor
        ...
        # else:
        #     hidden_states = self.norm1(hidden_states)
        hidden_states = self.norm1(hidden_states, local_conditions)
