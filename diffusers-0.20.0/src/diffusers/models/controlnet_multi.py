...
class ConditioningEmbeddingForVideo_40_Multi_new(nn.Module):
    def __init__(
        self,
        conditioning_embedding_channels: int = 128,
    ):
    ...
    def forward(self, conditioning):
        # [B, 256, 128] -> [B, 128, 256, 16]
        embedding = conditioning.transpose(1, 2)
        embedding = embedding.unsqueeze(-1).repeat(1, 1, 1, 16)
        # [B, 128, 256] -> [B, 128, 256, 16] repeat 16 times
        embedding = self.conv_out(embedding)
...
class ControlNetMultiModel(ModelMixin, ConfigMixin, FromOriginalControlnetMixin):
    """
    A ControlNet model.
    ...
    def __init__(
        ...
        # self.controlnet_cond_embedding = ConditioningEmbeddingForVideo_40_Multi()
        self.controlnet_cond_embedding = ConditioningEmbeddingForVideo_40_Multi_new()
    ...
    def forward(
        ...
        # sample [B, 8, 256, 16] -> [B, 128, 256, 16]
        # import pdb; pdb.set_trace()
        # controlnet_cond [B, 40, 768]
