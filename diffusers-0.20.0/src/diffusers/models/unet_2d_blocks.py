...
from .resnet import Downsample2D, FirDownsample2D, FirUpsample2D, KDownsample2D, KUpsample2D, ResnetBlock2D, Upsample2D, ResnetBlock2DFDN
...
def get_down_block(
    ...
    elif down_block_type == "CrossAttnDownBlock2DFDN":
        if cross_attention_dim is None:
            raise ValueError("cross_attention_dim must be specified for CrossAttnDownBlock2DFDN")
        return CrossAttnDownBlock2DFDN(
            num_layers=num_layers,
            transformer_layers_per_block=transformer_layers_per_block,
            in_channels=in_channels,
            out_channels=out_channels,
...
class CrossAttnDownBlock2DFDN(nn.Module):
    def __init__(
        self,
        in_channels: int,
        out_channels: int,
    ...
    def forward(
        self,
        hidden_states: torch.FloatTensor,
        local_conditions: torch.FloatTensor,
        temb: Optional[torch.FloatTensor] = None,
        encoder_hidden_states: Optional[torch.FloatTensor] = None,
        attention_mask: Optional[torch.FloatTensor] = None,
        cross_attention_kwargs: Optional[Dict[str, Any]] = None,
        ...
        for i, (resnet, attn) in enumerate(blocks):
            if self.training and self.gradient_checkpointing:
                ...
                if i == 0:
                    hidden_states = resnet(hidden_states, local_conditions, temb)
                else:
                    hidden_states = resnet(hidden_states, temb)
                hidden_states = attn(
                    hidden_states,
