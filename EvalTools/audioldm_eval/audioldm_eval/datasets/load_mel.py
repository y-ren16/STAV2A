    ...
    def __init__(
        ...
        if intersect_keys is not None:
            ...
            for x in os.listdir(datadir):
                #wav_name = x.replace(".wav", "")
                #print("wav_name out", wav_name)
                if x in intersect_keys:
                    #print("wav_name in", wav_name)
                    self.datalist.append(os.path.join(datadir, x))
        else:
            self.datalist = [os.path.join(datadir, x) for x in os.listdir(datadir)]
        # only wav
        self.datalist = [x for x in self.datalist if x.endswith(".wav")]
