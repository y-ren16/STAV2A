...
class EvaluationHelper:
    ...
    def main(
        ...
        same_name, intersections = self.get_filename_intersection_ratio(
            generate_files_path, groundtruth_path, limit_num=limit_num
        )

        #print("intersections", intersections)
        metrics = self.calculate_metrics(generate_files_path, groundtruth_path, same_name, intersections, limit_num=limit_num) # , recalculate=True
    ...
    def get_filename_intersection_ratio(
        ...
        data_dict1 = {}
        for data_gen in self.datalist1:
            if data_gen.endswith(".wav"):
                data_dict1[os.path.basename(data_gen)] = data_gen
        data_dict2 = {os.path.basename(x): x for x in self.datalist2}

        # for data_gt in data_dict2:
        #     assert data_gt in data_dict1, "The file %s is not in %s" % (data_gt, dir1)

        for data_gen in data_dict1:
            assert data_gen in data_dict2, "The file %s is not in %s" % (data_gen, dir2)
    ...
    def calculate_metrics(self, generate_files_path, groundtruth_path, same_name, intersect_keys, limit_num=None, calculate_psnr_ssim=False, calculate_lsd=False, recalculate=False):
        # Generation, target
        torch.manual_seed(0)
        ...
        print("11", generate_files_path, groundtruth_path, limit_num)
