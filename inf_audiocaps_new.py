...
def main():
    ...
    with open(args.test_file) as f:
        for line in f:
            ...
            if args.text_key == 'None':
                text_prompts.append("")
            elif input_file[args.text_key] is None:
            ...
            # wavnames.append(os.path.basename(input_file[args.video_key].replace(".mp4", ".wav")))
            ...
            wavnames.append(basename + ".wav")

    # with open(args.test_file) as f:
    #     data = f.read()
    # input_file = json.loads(data)
    # text_prompts = [line[args.text_key] for line in input_file["data"]]
    # wavnames = [os.path.basename(line["wav"]) for line in input_file["data"]]
    ...
    if num_samples == 1:
        ...
        for j, wav in enumerate(all_outputs):
            # basename = os.path.basename(wavnames[j])
            basename = basenames[j]
            sf.write("{}/{}".format(output_dir, wavnames[j]), wav, samplerate=16000)

            # try:
            #     video_target = VideoFileClip(video[j])
            #     duration = video_target.duration
            #     audio_target = librosa.load(f"{output_dir}/{wavnames[j]}", sr=16000)[0] 
